...
export class DrizzlePgliteCustomerRepository implements CustomerRepository {
  ...
  async create(
    ...
    try {
      ...
      return validate(customerSchema, normalizedCustomer).mapErr((error) => {
  ...
  async findById(
    ...
    try {
      ...
      const customerData = {
        ...
        location: result[0].location ?? undefined,
        foundedYear: result[0].foundedYear ?? undefined,
        website: result[0].website ?? undefined,
        description: result[0].description ?? undefined,
        parentCustomerId: result[0].parentCustomerId ?? undefined,
        assignedUserId: result[0].assignedUserId ?? undefined,
      };

      return validate(customerSchema, customerData).mapErr((error) => {
        return new RepositoryError("Invalid customer data", error);
      });
    } catch (error) {
        ...
        result
          .map((item) => {
            ...
            const normalizedItem = {
              ...
              parentCustomerId: item.parentCustomerId ?? undefined,
              assignedUserId: item.assignedUserId ?? undefined,
            };
            return validate(customerSchema, normalizedItem).unwrapOr(null);
          })
          .filter((item) => item !== null),
      );
    } catch (error) {
