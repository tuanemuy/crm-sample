...
export class DrizzlePgliteUserRepository implements UserRepository {
  ...
  async create(
    ...
    try {
      ...
      return validate(userSchema, userData).mapErr((error) => {
  ...
  async findById(id: string): Promise<Result<User | null, RepositoryError>> {
    try {
      ...
      const userData = {
        ...result[0],
        lastLoginAt: result[0].lastLoginAt ?? null,
      };
      ...
      return ok({
        items: items
          .map((item) => {
            ...
            return validate(userSchema, userData).unwrapOr(null);
          })
  ...
  async getProfile(
    ...
    try {
      ...
      const userValidation = validate(userSchema, userData);
