...
export class DrizzlePgliteCampaignRepository implements CampaignRepository {
  ...
  async list(
    ...
    const filters = [
      filter?.keyword
        ? sql`lower(${campaigns.name}) like lower(${`%${filter.keyword}%`})`
        : undefined,
  ...
  async assignLeads(
    ...
    try {
      ...
      return ok(
        result
          .map((item) =>
            validate(campaignLeadSchema, {
              ...item,
              lastContactedAt: item.lastContactedAt ?? undefined,
              responseAt: item.responseAt ?? undefined,
  ...
  async updateLeadStatus(
    ...
    try {
      ...
      return validate(campaignLeadSchema, {
        ...campaignLead,
        lastContactedAt: campaignLead.lastContactedAt ?? undefined,
        responseAt: campaignLead.responseAt ?? undefined,
        notes: campaignLead.notes ?? undefined,
      }).mapErr((error) => {
  ...
  async findCampaignLead(
    ...
    try {
      ...
      return validate(campaignLeadSchema, {
        ...result[0],
        lastContactedAt: result[0].lastContactedAt ?? undefined,
        responseAt: result[0].responseAt ?? undefined,
