...
export class DrizzlePgliteDealRepository implements DealRepository {
  constructor(private readonly db: Database) {}

  private transformDealData(
    deal: Record<string, unknown>,
    forDisplay = true,
  ): Record<string, unknown> {
    // Transform NULL values to undefined for optional fields
    // Transform amount to currency format only for display
    const amount =
      deal.amount && forDisplay
        ? this.formatCurrency(String(deal.amount))
        : deal.amount;
    return {
      ...deal,
      amount,
  ...
  private formatCurrency(amount: string): string {
    // Convert string to number, format with commas, and add $ prefix
    const num = Number.parseFloat(amount);
    if (Number.isNaN(num)) return amount;
    return `$${num.toLocaleString("en-US")}`;
  }

  private cleanAmountForDb(amount: string): string {
    // Remove currency formatting for database storage
    // Convert "$50,000" -> "50000"
  ...
  async create(
    ...
    try {
      // Ensure amount is in numeric string format for database insertion
      const cleanParams = {
        ...params,
        amount: this.cleanAmountForDb(params.amount),
      };
      ...
      const dealData = this.transformDealData(deal, false);

      return validate(dealSchema, dealData).mapErr((error) => {
  ...
  async findById(id: string): Promise<Result<Deal | null, RepositoryError>> {
    try {
      ...
      const dealData = this.transformDealData(result[0], false);
  ...
  async findByIdWithRelations(
    ...
    try {
      ...
      if (customerResult.length === 0) {
        return err(new RepositoryError("Deal customer not found"));
      }

      if (assignedUserResult.length === 0) {
        return err(new RepositoryError("Deal assigned user not found"));
      }

      // Transform database data to match domain schema
      const dealWithRelations = {
        ...this.transformDealData(deal, false),
  ...
  async list(
    ...
    try {
      ...
      return ok({
        items: items
          .map((item) => {
            const transformedItem = this.transformDealData(item, false);
            return validate(dealSchema, transformedItem).unwrapOr(null);
          })
  ...
  async update(
    ...
    try {
      ...
      const cleanParams = {
        ...params,
        ...(params.amount && { amount: this.cleanAmountForDb(params.amount) }),
      };
      const result = await this.db
        .update(deals)
        .set({ ...cleanParams, updatedAt: new Date() })
        .where(eq(deals.id, id))
        .returning();
