...
export class DrizzlePgliteActivityRepository implements ActivityRepository {
  ...
  async create(
    ...
    try {
      ...
      return validate(activitySchema, activityData).mapErr((error) => {
  ...
  async findById(
    ...
    try {
      ...
      const activityData = {
        ...
        scheduledAt: result[0].scheduledAt ?? undefined,
        dueDate: result[0].dueDate ?? undefined,
        completedAt: result[0].completedAt ?? undefined,
        duration: result[0].duration ?? undefined,
        customerId: result[0].customerId ?? undefined,
        contactId: result[0].contactId ?? undefined,
        dealId: result[0].dealId ?? undefined,
        leadId: result[0].leadId ?? undefined,
      };
      ...
      return ok({
        items: items
          .map((item) => {
            ...
            return validate(activitySchema, activityData).unwrapOr(null);
          })
  ...
  async update(
    ...
    try {
      // Convert undefined to null for database update when field is explicitly being set
      const dbParams: Record<string, unknown> = {
        ...params,
        updatedAt: new Date(),
      };

      // Only convert undefined to null for fields that are explicitly in the params
      if ("description" in params) {
        dbParams.description =
          params.description === undefined ? null : params.description;
      }
      if ("scheduledAt" in params) {
        dbParams.scheduledAt =
          params.scheduledAt === undefined ? null : params.scheduledAt;
      }
      if ("dueDate" in params) {
        dbParams.dueDate = params.dueDate === undefined ? null : params.dueDate;
      }
      if ("completedAt" in params) {
        dbParams.completedAt =
          params.completedAt === undefined ? null : params.completedAt;
      }
      if ("duration" in params) {
        dbParams.duration =
          params.duration === undefined ? null : params.duration;
      }
      if ("customerId" in params) {
        dbParams.customerId =
          params.customerId === undefined ? null : params.customerId;
      }
      if ("contactId" in params) {
        dbParams.contactId =
          params.contactId === undefined ? null : params.contactId;
      }
      if ("dealId" in params) {
        dbParams.dealId = params.dealId === undefined ? null : params.dealId;
      }
      if ("leadId" in params) {
        dbParams.leadId = params.leadId === undefined ? null : params.leadId;
      }
