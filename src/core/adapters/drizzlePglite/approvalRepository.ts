...
export class DrizzlePgliteApprovalRepository implements ApprovalRepository {
  ...
  async findByIdWithRelations(
    ...
    try {
      ...
      const result = await this.db
        ...
        .leftJoin(
          requestedByUser,
          eq(approvals.requestedBy, requestedByUser.id),
        )
        .leftJoin(assignedToUser, eq(approvals.assignedTo, assignedToUser.id))
        .where(eq(approvals.id, id))
        .limit(1);
  ...
  async list(
    ...
    // Handle priority sorting with proper order: urgent > high > medium > low
    let sortColumn;
    if (sortBy === "priority") {
      sortColumn = sql`
