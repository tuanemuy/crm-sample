...
export class DrizzlePgliteContactHistoryRepository
  implements ContactHistoryRepository
{
  ...
  async getById(
    ...
    try {
      const result = await this.db
        ...
        .innerJoin(users, eq(contactHistory.contactedByUserId, users.id))
      ...
      // Transform null values to undefined for proper validation
      const transformedRecord = {
        ...record,
        content: record.content === null ? undefined : record.content,
        direction: record.direction === null ? undefined : record.direction,
        status: record.status === null ? undefined : record.status,
        duration: record.duration === null ? undefined : record.duration,
        contact: record.contact === null ? undefined : record.contact,
        customer: record.customer === null ? undefined : record.customer,
        contactedByUser:
          record.contactedByUser === null ? undefined : record.contactedByUser,
      };
  ...
  > {
    ...
    // Use sortBy/sortOrder from top level, or fall back to pagination object
    const sortField = sortBy || pagination.orderBy || "contactedAt";
    const sortDirection = sortOrder || pagination.order || "desc";
    ...
    const orderColumn =
      sortField === "contactedAt"
        ? contactHistory.contactedAt
        : sortField === "subject"
          ? contactHistory.subject
          : sortField === "type"
            ? contactHistory.type
            : sortField === "createdAt"
              ? contactHistory.createdAt
              : contactHistory.contactedAt; // default
      ...
      const validatedItems = items
        .map((item) => {
          ...
          const transformedItem = {
            ...
            direction: item.direction === null ? undefined : item.direction,
            status: item.status === null ? undefined : item.status,
            duration: item.duration === null ? undefined : item.duration,
            contact: item.contact === null ? undefined : item.contact,
            customer: item.customer === null ? undefined : item.customer,
            contactedByUser:
              item.contactedByUser === null ? undefined : item.contactedByUser,
          };
