...
export class DrizzlePgliteContactRepository implements ContactRepository {
  ...
  async create(
    ...
    try {
      ...
      return validate(contactSchema, contactData).mapErr((error) => {
  ...
  async findById(id: string): Promise<Result<Contact | null, RepositoryError>> {
    try {
      ...
      const contactData = {
        ...
        department: result[0].department ?? undefined,
        email: result[0].email ?? undefined,
        phone: result[0].phone ?? undefined,
        mobile: result[0].mobile ?? undefined,
      };
  ...
  async list(
    ...
    const filters = [
      filter?.keyword
        ? or(
            sql`${contacts.name} ILIKE ${`%${filter.keyword}%`}`,
            sql`${contacts.email} ILIKE ${`%${filter.keyword}%`}`,
          )
        : undefined,
      filter?.customerId
    ...
    const sortField = sortBy || pagination.orderBy;
    const sortDirection = sortOrder || pagination.order;
    ...
    const orderDirection =
      sortDirection === "asc" ? asc(orderBy) : desc(orderBy);
      ...
      return ok({
        items: items
          .map((item) => {
            ...
            return validate(contactSchema, contactData).unwrapOr(null);
          })
  ...
  async update(
    ...
    try {
      ...
      // Convert undefined values to null for proper database handling
      const updateData = Object.entries(params).reduce(
        (acc, [key, value]) => ({
          ...acc,
  ...
  async search(
    ...
    try {
      const baseFilters = [
        or(
          sql`${contacts.name} ILIKE ${`%${keyword}%`}`,
          sql`${contacts.email} ILIKE ${`%${keyword}%`}`,
          sql`${contacts.title} ILIKE ${`%${keyword}%`}`,
        ),
        eq(contacts.isActive, true),
      ];
