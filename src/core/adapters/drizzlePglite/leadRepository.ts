...
export class DrizzlePgliteLeadRepository implements LeadRepository {
  ...
  async create(
    ...
    try {
      ...
      return validate(leadSchema, leadData).mapErr((error) => {
  ...
  async findById(id: string): Promise<Result<Lead | null, RepositoryError>> {
    try {
      ...
      const leadData = {
        ...
        industry: result[0].industry ?? undefined,
        source: result[0].source ?? undefined,
        notes: result[0].notes ?? undefined,
        assignedUserId: result[0].assignedUserId ?? undefined,
        convertedCustomerId: result[0].convertedCustomerId ?? undefined,
        convertedAt: result[0].convertedAt ?? undefined,
      };
  ...
  async list(
    ...
    const filters = [
      filter?.keyword
        ? sql`(LOWER(${leads.firstName}) LIKE LOWER(${`%${filter.keyword}%`}) OR LOWER(${leads.lastName}) LIKE LOWER(${`%${filter.keyword}%`}) OR LOWER(${leads.company}) LIKE LOWER(${`%${filter.keyword}%`}) OR LOWER(${leads.email}) LIKE LOWER(${`%${filter.keyword}%`}))`
        : undefined,
      ...
      filter?.createdBefore
        ...
        : undefined,
      // Tags filtering - currently disabled due to PGlite array operator compatibility
      // TODO: Implement tags filtering when array operators are supported
      // filter?.tags && filter.tags.length > 0
      //   ? sql`${leads.tags} && ARRAY[...]`
      //   : undefined,
      ...
      const processedItems = items
        .map((item) => {
          ...
          const leadData = {
            ...
            convertedCustomerId: item.convertedCustomerId ?? undefined,
            convertedAt: item.convertedAt ?? undefined,
          };
          return validate(leadSchema, leadData).unwrapOr(null);
        })
      ...
      return ok(
        result
          .map((item) => {
            ...
            return validate(leadSchema, normalizedItem).unwrapOr(null);
          })
          .filter((item) => item !== null),
      );
    } catch (error) {
